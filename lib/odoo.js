...
// searchRead: forma clara de leer un modelo.
// domain = lista de condiciones, ej: [['user_id','=',47],['active','=',true]]
// opts   = { fields: [...], limit, order }
// Esta función se encarga de envolver el domain en el formato exacto que
// espera execute_kw, para no tener que contar corchetes en cada llamada.
export async function searchRead(model, domain = [], opts = {}) {
  const uid = await getOdooUid();
  return odooRpc('object', 'execute_kw', [
    ODOO_DB, uid, ODOO_API_KEY,
    model, 'search_read',
    [domain],          // <- el domain va envuelto UNA sola vez, aquí
    opts,              // <- fields / limit / order
  ]);
}
